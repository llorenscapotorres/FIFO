...
from app.models import PurchaseLot, SaleConsumption
...
def list_lots(asset_id: int, db: Session = Depends(get_db)):
    position.get_asset_or_404(db, asset_id)
    return position.lot_summaries(db, asset_id)


@router.post("", response_model=PurchaseLotRead, status_code=201)
def create_lot(asset_id: int, payload: PurchaseLotCreate, db: Session = Depends(get_db)):
    position.get_asset_or_404(db, asset_id)
...
def update_lot(asset_id: int, lot_id: int, payload: PurchaseLotUpdate, db: Session = Depends(get_db)):
...
def delete_lot(asset_id: int, lot_id: int, db: Session = Depends(get_db)):
    lot = _get_lot_or_404(db, asset_id, lot_id)
